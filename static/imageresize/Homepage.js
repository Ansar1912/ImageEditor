...
document.querySelector(".dropzone-input").addEventListener('change', function (e) {
    ...
    for (let i = 0; i < inputfiles.files.length; i++) {
        ...
        if ((file = this.files[i])) {
            ...
            img.onload = function () {
                ...
                $('.panel .widthinput input').val(Math.round(totalwidth / inputfiles.files.length))
                $('.panel .heightinput input').val(Math.round(totalheight / inputfiles.files.length))
            };
        }
    }
    filedetails(inputfiles.files)
